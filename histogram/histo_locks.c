...
long* histogram(char* fn_input, int num_threads) {
  pImage image;
  int i, j, m;
  long* histo;
  ...
  // init all locks unlocked
  omp_lock_t ompLocks[256];
  for (i = 0; i < 256; i++) {
  ...
  for (m=0; m<100; m++) {
    ...
    for (i=0; i<image->row; i++) {
      ...
      for (j=0; j<image->col; j++) {
	omp_set_lock(&ompLocks[image->content[i][j]]);
        histo[image->content[i][j]]++;
	omp_unset_lock(&ompLocks[image->content[i][j]]);
      }//for j
  ...
  //finsh compute, destroy all the locks
  for (i = 0; i < 256; i++) {
...
int main(int argc, char** argv)
{
  ...
  histo = histogram(argv[1], atoi(argv[2]));
  return 0;
}
